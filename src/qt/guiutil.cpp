...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("millionaire"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("millionaire://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "millionaire:");
    }
...
{
    QString ret = QString("millionaire:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "millionaire.desktop";
    return GetAutostartDir() / strprintf("millionaire-%s.lnk", chain);
}
